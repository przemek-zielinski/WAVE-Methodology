...
---
*Generated by WAVE Living Patterns Pipeline — PULSE R1 EN | {today}*
...
"""
    ...
    log(f"Pausing {RATE_LIMIT_PAUSE}s before R1 translation...")
    import time
    time.sleep(RATE_LIMIT_PAUSE)

    # --- Translation to Polish ---
    log("Translating R1 to Polish...")
    translate_prompt = TRANSLATE_PROMPT.format(document=result)
    result_pl = call_api(client, translate_prompt, model=SONNET, max_tokens=12000, use_web_search=False)
    result_pl = cleanup_markdown(result_pl)
    log(f"R1 PL: {len(result_pl)} chars")

    comment_pl = f"""## 🫀 PULSE Runda 1 — Fundament (Polski)

{result_pl}

---
*Wygenerowano przez WAVE Living Patterns Pipeline — PULSE R1 PL | {today}*
*Następny krok: przejrzyj powyżej. Dodaj label `r1-approved` żeby przejść do Rundy 2.*
"""

    add_comment(issue_number, comment_pl)
    add_label(issue_number, "r1-ready")
    log("R1 PL posted. Label 'r1-ready' added.")
...
def run_r2(client, issue_number, comments, area, objective_function, context, today):
    ...
    # Cleanup markdown (tables + orphan dots + broken lines + orphan bullets)
    result = cleanup_markdown(result)
...
---
*Generated by WAVE Living Patterns Pipeline — PULSE R2 EN | {today}*
...
"""
    ...
    # --- Pause for rate limit ---
    log(f"Pausing {RATE_LIMIT_PAUSE}s before R2 translation...")
    import time
    time.sleep(RATE_LIMIT_PAUSE)

    # --- Translation to Polish ---
    log("Translating R2 to Polish...")
    translate_prompt = TRANSLATE_PROMPT.format(document=result)
    result_pl = call_api(client, translate_prompt, model=SONNET, max_tokens=8000, use_web_search=False)
    result_pl = cleanup_markdown(result_pl)
    log(f"R2 PL: {len(result_pl)} chars")

    comment_pl = f"""## 🫀 PULSE Runda 2 — Delta Weryfikacji (Polski)

{result_pl}

---
*Wygenerowano przez WAVE Living Patterns Pipeline — PULSE R2 PL | {today}*
*Następny krok: przejrzyj powyżej. Dodaj label `r2-approved` żeby przejść do Rundy 3 (Finał).*
"""

    add_comment(issue_number, comment_pl)
    add_label(issue_number, "r2-ready")
    log("R2 PL posted. Label 'r2-ready' added.")
...
def run_r3(client, issue_number, comments, area, objective_function, context, today):
    ...
    # Cleanup markdown (tables + orphan dots)
    result_en = cleanup_markdown(result_en)
...
"""
    ...
    # Cleanup markdown (tables + orphan dots) in PL
    result_pl = cleanup_markdown(result_pl)
